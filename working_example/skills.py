...
class Multiply(FunctionCallSkill):
    def __init__(
            ...
            function_args=[
                SkillArgAttr(name="a", dtype="float", description="First number", required=True),
                SkillArgAttr(name="b", dtype="float", description="Second number", required=True),
            ],
            visible_to_human: bool = True
    ):
        super().__init__(
            name=name, description=description, function_args=function_args, visible_to_human=visible_to_human
        )
