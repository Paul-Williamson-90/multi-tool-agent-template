...
class Context(ABC):
    ...
    def extract(self, instructions: str, llm: LLM) -> ExtractedFacts:
        ...
        def worker(page) -> tuple[int, ExtractedFacts]:
            return page, structured_invocation(
                ...
                context=EXTRACT_TEMPLATE.format(
                    brief=instructions,
                    contexts=self.show_content_by_page(page)
                ),
        ...
        with ThreadPoolExecutor(max_workers=self._max_workers) as executor:
            futures = {executor.submit(worker, page): page for page in range(1, self.pages + 1)}
        ...
        combined_facts = ExtractedFacts(facts=[fact for facts in extracted_facts_list for fact in facts.facts])
    ...
    def summarise(self, query: str, llm: LLM) -> str:
        ...
        if len(content_list) > self._max_content_in_summarise:
            indexes = np.random.choice(len(content_list), self._max_content_in_summarise, replace=False)
            indexes.sort()
            content_list = [content_list[i] for i in indexes]
            summarised_str = "**Too many documents to show, only showing a random selection**"
